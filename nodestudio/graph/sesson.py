...
class Session:
    ...
    async def run(id, operations):
        ...
        print('Run Sesson')
        await websocket.emit('status', { 'session_id':id, 'status':'compute', 'message':'Computing Nodes ...' })
        ...
        await websocket.emit('status', { 'session_id':id, 'status':'end', 'message':'Computation complete', 'time': process_time })
