...
def GRAPPArecon(data, R = 2, width = 5, height = 4):    
    ...
    kern  = GRAPPA(height,width, nc, R, ACS_length, ny,nx)
    inMatrix = np.zeros([kern.num_train, kern.kernelsize], dtype = complex)
    outMatrix = np.zeros([kern.num_train, kern.num_target], dtype = complex)
    # the self-calibration technique will find the weights that is most consistent with the ACS data in the LSE sense 
    num = 0
    for y in ACS_index[0][0:kern.num_vertical]:
        ys = np.linspace(y, y+kern.kernelheight-1, height, dtype = int)
        for x in range(nx):
            x_index = kern._get_xIndex_(x)
            in_temp = dataR[ys][:,x_index][:,:,:]
            y_index = kern._get_yIndex_(ys)
    ...
    for y in fill_index:
        ...
        for x in range(nx):
            ...
            fill_yidx = kern._get_yIndex_(y_index)
